/* ============================================================
   4. TECHNICAL — Monospace, dev/terminal aesthetic
   ============================================================ */
function TechnicalTemplate({ data, accent, lang, sections, onEdit }) {
  ...
  const Comment = ({ children }) => (
    <span style={{ color: dimColor, fontStyle: 'italic' }}>{children}</span>
  );

  const Hash = () => <span style={{ color: accent, fontWeight: 700 }}>##</span>;
    ...
    personal: () => (
      <header key="personal" style={{ marginBottom: 22, paddingBottom: 14, borderBottom: `2px solid ${accent}` }}>
        ...
        <Editable as="h1" {...F(['personal','name'])}
          style={{
            fontFamily: monoFont, fontSize: 30, fontWeight: 700,
            margin: 0, color: inkColor, letterSpacing: '-0.01em', lineHeight: 1.05,
          }}/>
    ...
    summary: () => (
      <Section key="summary" id="summary" title={t.sections.summary}>
        <div style={{ fontFamily: monoFont, fontSize: 10.5, lineHeight: 1.65, color: '#333', background: codeBg, padding: '10px 14px', borderInlineStart: `3px solid ${accent}` }}>
          <Comment>{isRtl ? '/* תקציר אישי */' : '/* about */'}</Comment>
          ...
          {(data.experience || []).map((j, i) => (
            <div key={i}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'baseline' }}>
                ...
                <span style={{ fontSize: 10, color: dimColor }}>[{j.start} → {j.end || (isRtl ? 'now' : 'now')}]</span>
              </div>
              ...
              <span>
                {(sk.items || []).map((it, k) => (
                  <React.Fragment key={k}>
                    <span style={{ background: codeBg, padding: '1px 6px', borderRadius: 3, fontSize: 10, marginInlineEnd: 4, border: '1px solid #E5E0D8' }}>
                      <Editable
                        value={it}
                        onChange={(v) => onEdit(['skills',i,'items'], (sk.items || []).map((ii, kk) => kk === k ? v : ii))}
                      />
                    </span>
                  </React.Fragment>
                ))}
...
window.TechnicalTemplate = TechnicalTemplate;
