...
// Breeze: Clean modern split header, minimal section labels with accent underline
const BreezeTemplate = memo(function BreezeTemplate({ data, accent, lang, sections, showPhoto, onEdit }) {
  const F = makeField(data, onEdit);
  const t = I18N[lang];
  const isRtl = lang === 'he';

  const Section = ({ id, title, children }) => (
    <section data-section={id} style={{ marginBottom: 11 }}>
      <h2 style={{
        fontSize: 9, letterSpacing: '0.2em', textTransform: 'uppercase',
        fontWeight: 600, color: accent, margin: '0 0 6px',
        paddingBottom: 4, borderBottom: `1px solid ${accent}`,
  ...
  const renderMap = {
    personal: () => (
      <header key="personal" style={{ marginBottom: 14 }}>
        ...
        <div style={{ flex:1, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16 }}>
          <div>
            <Editable as="h1" {...F(['personal', 'name'])}
              style={{ fontFamily: 'var(--cv-heading)', fontSize: 36, fontWeight: 600, margin: 0, letterSpacing: '-0.02em', lineHeight: 1.05, color: '#111' }}/>
            <Editable as="div" {...F(['personal', 'title'])}
              style={{ fontSize: 12.5, color: '#555', marginTop: 5, fontWeight: 400, letterSpacing: '0.01em' }}/>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 3, fontSize: 10, color: '#555', textAlign: isRtl ? 'left' : 'right', flexShrink: 0, paddingTop: 4 }}>
        ...
        <div style={{ height: 1, background: '#D0CCC4', marginTop: 12 }}/>
      </header>
    ),
    summary: () => (
          ...
          {(data.skills || []).map((sk, i) => (
            <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'baseline', fontSize: 11 }}>
              <Editable value={sk.category} onChange={(v) => onEdit(['skills', i, 'category'], v)}
                style={{ fontWeight: 700, color: accent, minWidth: 70, fontSize: 10.5, flexShrink: 0 }}/>
      ...
      <Section key="certifications" id="certifications" title={t.sections.certifications}>
        {(data.certifications || []).map((c, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, marginBottom: 4 }}>
            ...
            <Editable value={c.date} onChange={(v) => onEdit(['certifications', i, 'date'], v)} style={{ color: '#888' }}/>
          </div>
        ))}
      </Section>
    ),
      ...
      <Section key="awards" id="awards" title={t.sections.awards}>
        {(data.awards || []).map((a, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, marginBottom: 5 }}>
            ...
            <Editable value={a.date} onChange={(v) => onEdit(['awards', i, 'date'], v)} style={{ color: '#888', whiteSpace: 'nowrap' }}/>
          </div>
        ))}
      </Section>
    ),
          ...
          <div key={i} style={{ marginBottom: 6, fontSize: 11 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 8 }}>
              ...
              <Editable value={p.date} onChange={(v) => onEdit(['publications', i, 'date'], v)}
                style={{ color: '#999', whiteSpace: 'nowrap', fontSize: 10 }}/>
            </div>
